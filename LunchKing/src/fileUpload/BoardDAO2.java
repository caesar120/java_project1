...
public class BoardDAO2 { 
	...
	public boolean updateDB(Board2 board) {
		connect();		
	...
	// 신규 부서 추가 메서드
	public boolean insertDB(Board2 board) {
		connect();				
		...
		try {
			...
			pstmt.setString(5, board.getUploadedfileName());
			pstmt.executeUpdate();
		} catch (SQLException e) {
			e.printStackTrace();
			return false;
	...
	// 전체 부서 목록을 가져오는 메서드
	public ArrayList<Board2> getDBList() {
		connect();
		ArrayList<Board2> datas = new ArrayList<Board2>();
